...
int smb380_init(smb380_t *smb380) {
	...
	comres +=
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_CHIP_ID__REG, &data, 1); /* read Chip Id */
...
int smb380_soft_reset() {
	...
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_SOFT_RESET__REG, &data,1);
	return comres;
}
...
int smb380_update_image() {
	...
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_UPDATE_IMAGE__REG, &data,1);
	return comres;
}
...
int smb380_set_image(smb380regs_t *smb380Image) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres = SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_EE_W__REG,&data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_EE_W, SMB380_EE_W_ON);
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_EE_W__REG, &data, 1);
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_IMAGE_BASE, (unsigned char*)smb380Image, SMB380_IMAGE_LEN);
	comres = SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_EE_W__REG,&data, 1);
...
int smb380_get_image(smb380regs_t *smb380Image) {
	...
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_EE_W__REG, &data, 1);
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_IMAGE_BASE, (unsigned char *)smb380Image, SMB380_IMAGE_LEN);
	data = SMB380_SET_BITSLICE(data, SMB380_EE_W, SMB380_EE_W_OFF);
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_EE_W__REG, &data, 1);
	return comres;
...
int smb380_get_offset(unsigned char xyz, unsigned short *offset) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, (SMB380_OFFSET_X_LSB__REG+xyz), &data, 1);
	data = SMB380_GET_BITSLICE(data, SMB380_OFFSET_X_LSB);
	*offset = data;
	comres +=
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, (SMB380_OFFSET_X_MSB__REG+xyz), &data, 1);
	*offset |= (data << 2);
	return comres;
}
...
int smb380_set_offset(unsigned char xyz, unsigned short offset) {
	...
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, (SMB380_OFFSET_X_LSB__REG+xyz), &data, 1);
	data = (offset & 0x3ff) >> 2;
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, (SMB380_OFFSET_X_MSB__REG+xyz), &data, 1);
	return comres;
}
...
int smb380_set_offset_eeprom(unsigned char xyz, unsigned short offset) {
	...
	data = SMB380_SET_BITSLICE(data, SMB380_OFFSET_X_LSB, offset);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, (SMB380_EEP_OFFSET+SMB380_OFFSET_X_LSB__REG + xyz), &data, 1);
	p_smb380->delay_msec(34);
	data = (offset & 0x3ff) >> 2;
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, (SMB380_EEP_OFFSET+ SMB380_OFFSET_X_MSB__REG+xyz), &data, 1);
	p_smb380->delay_msec(34);
	return comres;
}
...
int smb380_get_ee_w(unsigned char *eew) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres = SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_EE_W__REG,eew, 1);
...
int smb380_selftest(unsigned char st) {
	...
	unsigned char data;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_SELF_TEST__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_SELF_TEST, st);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_SELF_TEST__REG, &data, 1);
	return comres;
...
int smb380_set_range(char range) {
	...
	if (range < 3) {
		comres =
				SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_RANGE__REG, &data, 1 );
		data = SMB380_SET_BITSLICE(data, SMB380_RANGE, range);
		comres +=
				SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_RANGE__REG, &data, 1);
	}
	return comres;
...
int smb380_get_range(unsigned char *range) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_RANGE__REG, range, 1 );
...
int smb380_set_mode(unsigned char mode) {
	...
	if (mode < 4 && mode != 1) {
		comres =
				SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_WAKE_UP__REG, &data1, 1 );
		comres +=
				SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_SLEEP__REG, &data2, 1 );
		switch (mode) {
		case SMB380_MODE_NORMAL:
			data1 = SMB380_SET_BITSLICE(data1, SMB380_WAKE_UP, 0);
			data2 = SMB380_SET_BITSLICE(data2, SMB380_SLEEP, 0);
			break;
		case SMB380_MODE_SLEEP:
			data1 = SMB380_SET_BITSLICE(data1, SMB380_WAKE_UP, 0);
			data2 = SMB380_SET_BITSLICE(data2, SMB380_SLEEP, 1);
			break;
		case SMB380_MODE_WAKE_UP:
			data1 = SMB380_SET_BITSLICE(data1, SMB380_WAKE_UP, 1);
			data2 = SMB380_SET_BITSLICE(data2, SMB380_SLEEP, 0);
			break;
		default:
			data1 = SMB380_SET_BITSLICE(data1, SMB380_WAKE_UP, 0);
		...
		comres +=
				SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_WAKE_UP__REG, &data1, 1);
		comres +=
				SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_SLEEP__REG, &data2, 1);
		p_smb380->mode = mode;
	}
	return comres;
...
int smb380_set_bandwidth(char bw) {
	...
	if (bw < 8) {
		...
		comres =
				SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_BANDWIDTH__REG, &data, 1 );
		data = SMB380_SET_BITSLICE(data, SMB380_BANDWIDTH, bw);
		comres +=
				SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_BANDWIDTH__REG, &data, 1 );
...
int smb380_get_bandwidth(unsigned char *bw) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_BANDWIDTH__REG, bw, 1 );
...
int smb380_set_wake_up_pause(unsigned char wup) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_WAKE_UP_PAUSE__REG, &data, 1 );
	data = SMB380_SET_BITSLICE(data, SMB380_WAKE_UP_PAUSE, wup);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_WAKE_UP_PAUSE__REG, &data, 1 );
	return comres;
}
...
int smb380_set_low_g_threshold(unsigned char th) {
	...
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_LG_THRES__REG, &th, 1);
	return comres;
...
int smb380_get_low_g_threshold(unsigned char *th) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_LG_THRES__REG, th, 1);
...
int smb380_set_low_g_countdown(unsigned char cnt) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_COUNTER_LG__REG, &data, 1 );
	data = SMB380_SET_BITSLICE(data, SMB380_COUNTER_LG, cnt);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_COUNTER_LG__REG, &data, 1 );
	return comres;
}
...
int smb380_set_low_g_hysteresis(unsigned char hyst) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_LG_HYST__REG, &data, 1 );
	data = SMB380_SET_BITSLICE(data, SMB380_LG_HYST, hyst);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_LG_HYST__REG, &data, 1 );
	return comres;
}
...
int smb380_set_high_g_countdown(unsigned char cnt) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_COUNTER_HG__REG, &data, 1 );
	data = SMB380_SET_BITSLICE(data, SMB380_COUNTER_HG, cnt);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_COUNTER_HG__REG, &data, 1 );
	return comres;
}
...
int smb380_set_low_g_duration(unsigned char dur) {
	...
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_LG_DUR__REG, &dur, 1);
...
int smb380_set_high_g_hysteresis(unsigned char hyst) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_HG_HYST__REG, &data, 1 );
	data = SMB380_SET_BITSLICE(data, SMB380_HG_HYST, hyst);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_HG_HYST__REG, &data, 1 );
	return comres;
}
...
int smb380_get_low_g_duration(unsigned char *dur) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_LG_DUR__REG, dur, 1);
	return comres;
...
int smb380_set_high_g_threshold(unsigned char th) {
	...
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_HG_THRES__REG, &th, 1);
	return comres;
...
int smb380_get_high_g_threshold(unsigned char *th) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_HG_THRES__REG, th, 1);
...
int smb380_set_high_g_duration(unsigned char dur) {
	...
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_HG_DUR__REG, &dur, 1);
	return comres;
}
...
int smb380_get_high_g_duration(unsigned char *dur) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_HG_DUR__REG, dur, 1);
...
int smb380_set_any_motion_threshold(unsigned char th) {
	...
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_ANY_MOTION_THRES__REG, &th, 1);
...
int smb380_get_any_motion_threshold(unsigned char *th) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ANY_MOTION_THRES__REG, th, 1);
...
int smb380_set_any_motion_count(unsigned char amc) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ANY_MOTION_DUR__REG, &data, 1 );
	data = SMB380_SET_BITSLICE(data, SMB380_ANY_MOTION_DUR, amc);
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_ANY_MOTION_DUR__REG, &data, 1 );
	return comres;
}
...
int smb380_set_interrupt_mask(unsigned char mask) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_CONF1_REG, &data[1], 1);
	comres +=
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_CONF2_REG, &data[3], 1);
	...
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_CONF1_REG, &data[1], 1);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_CONF2_REG, &data[3], 1);
...
int smb380_get_interrupt_mask(unsigned char *mask) {
	...
	SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_CONF1_REG, &data, 1);
	*mask = data & SMB380_CONF1_INT_MSK;
	SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_CONF2_REG, &data, 1);
...
int smb380_reset_interrupt(void) {
	...
	comres =
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_RESET_INT__REG, &data, 1);
	return comres;
...
int smb380_read_accel_x(short *a_x) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ACC_X_LSB__REG, data, 2);
...
int smb380_read_accel_y(short *a_y) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ACC_Y_LSB__REG, data, 2);
...
int smb380_read_accel_z(short *a_z) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ACC_Z_LSB__REG, data, 2);
...
int smb380_read_temperature(unsigned char * temp) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_TEMPERATURE__REG, temp, 1);
	return comres;
...
int smb380_read_accel_xyz(smb380acc_t * acc) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ACC_X_LSB__REG, &data[0],6);
...
int smb380_get_interrupt_status(unsigned char * ist) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres = SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_STATUS_REG, ist, 1);
	return comres;
}
...
int smb380_set_low_g_int(unsigned char onoff) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ENABLE_LG__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_ENABLE_LG, onoff);

	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_ENABLE_LG__REG, &data, 1);
...
int smb380_set_high_g_int(unsigned char onoff) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ENABLE_HG__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_ENABLE_HG, onoff);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_ENABLE_HG__REG, &data, 1);
...
int smb380_set_any_motion_int(unsigned char onoff) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_EN_ANY_MOTION__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_EN_ANY_MOTION, onoff);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_EN_ANY_MOTION__REG, &data, 1);
...
int smb380_set_alert_int(unsigned char onoff) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ALERT__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_ALERT, onoff);

	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_ALERT__REG, &data, 1);
...
int smb380_set_advanced_int(unsigned char onoff) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_ENABLE_ADV_INT__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_ENABLE_ADV_INT, onoff);

	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_ENABLE_ADV_INT__REG, &data, 1);
...
int smb380_latch_int(unsigned char latched) {
	...
	if (p_smb380 == 0)
		return E_SMB_NULL_PTR;
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_LATCH_INT__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_LATCH_INT, latched);

	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_LATCH_INT__REG, &data, 1);
...
int smb380_set_new_data_int(unsigned char onoff) {
	...
	comres =
			SMB380_BUS_READ_FUNC(p_smb380->dev_addr, SMB380_NEW_DATA_INT__REG, &data, 1);
	data = SMB380_SET_BITSLICE(data, SMB380_NEW_DATA_INT, onoff);
	comres +=
			SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, SMB380_NEW_DATA_INT__REG, &data, 1);
...
int smb380_read_reg(unsigned char addr, unsigned char *data, unsigned char len) {
	...
	comres = SMB380_BUS_READ_FUNC(p_smb380->dev_addr, addr, data, len);
	return comres;
...
int smb380_write_reg(unsigned char addr, unsigned char *data, unsigned char len) {
	...
	comres = SMB380_BUS_WRITE_FUNC(p_smb380->dev_addr, addr, data, len);
